...
@section('actions')
    <a href="{{ route('blog.admin.index') }}" class="btn btn-sm btn-outline-primary my-auto mx-3">
        ...
        <div class="row justify-content-center">
            <div class="col-md-10">
                <h5 class="text-muted small text-uppercase font-weight-bold mt-2">
                    {{ __('blog::stats.details.published') }} {{ \Carbon\Carbon::parse($data['post']->published_at)->format('F d, Y') }}
                </h5>
                ...
                @if($data['traffic'])
                    @foreach($data['traffic'] as $host => $views)
                        <div class="d-flex @if($loop->first) border-top @endif py-2 align-items-center">
                            <div class="mr-auto">
                                <p class="mb-0 py-1">
                                    @unless($host == __('blog::stats.details.referer.other'))
                                    ...
                                    @else
                                        ...
                                        <a data-toggle="tooltip" data-placement="right"
                                                       ...
                                                       title="{{ __('blog::stats.details.referer.unknown') }}">
                            ...
                            <div class="ml-auto">
                                <span class="text-muted">{{ \Modules\Blog\Helper::suffixNumber($views) }} {{ __('blog::stats.views') }}</span>
                            </div>
                        </div>
                    @endforeach
                @else
                    <p class="py-4 border-top"><em>{{ __('blog::stats.details.empty') }}</em>
                    </p>
                @endif
            </div>
            ...
            <div class="col-md-5 mt-4">
                <h5 class="text-muted small text-uppercase font-weight-bold">
                    {{ __('blog::stats.details.reading.header') }}
                </h5>
                ...
                @else
                    <p class="py-4 border-top">
                        <em>{{ __('blog::stats.details.empty') }}</em>
                    </p>
                @endif
            </div>
        </div>
