...
class Post extends Model
{
    ...
    {
        return $this->belongsToMany(Tag::class, 'blog_posts_tags', 'post_id', 'tag_id');
    }
    ...
    {
        return $this->belongsToMany(Topic::class, 'blog_posts_topics', 'post_id', 'topic_id');
    }
    ...
    {
        return $this->belongsTo(Modules\Account\Models\User::class);
    }
    ...
    public function getReadTimeAttribute(): string
    {
        ...
        return sprintf('%d %s %s', $minutes, Str::plural(__('blog::stats.details.reading.time'), $minutes), __('blog::stats.details.reading.read'));
    }
    ...
    public function getTopReferersAttribute(): array
    {
        ...
        $collection = collect();
        $data->each(function ($item, $key) use ($collection) {
            is_null($item->referer) ? $collection->push(__('blog::stats.details.referer.other')) : $collection->push(parse_url($item->referer)['host']);
        });
